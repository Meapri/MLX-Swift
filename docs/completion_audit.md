...
| Video-capable model generation works end-to-end | `MLXVLM_REAL_QWEN25VL_MODEL=/Users/naen/Models/mlx-community-Qwen2.5-VL-3B-Instruct-3bit SWIFT_BUILD_JOBS=18 scripts/verify_real_qwen25vl_video_smoke.sh` passed after downloading `mlx-community/Qwen2.5-VL-3B-Instruct-3bit` locally | Done for Qwen2.5-VL 3B 3bit |
...
| Python `mlx-vlm` model-family parity is complete | Gemma4 real smoke passes; Qwen2.5-VL video smoke passes for a local 3B 3bit model; Qwen VL planning/metadata is broad; non-generative families are classified | Partial |
