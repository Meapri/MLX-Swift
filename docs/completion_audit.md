...
| Python `mlx-vlm` model-family parity is complete | Gemma4 real smoke passes; Qwen2.5-VL tokenizer/image/video/stream smoke passes for a local 3B 3bit model; Qwen VL planning/metadata is broad; non-generative families are classified | Partial |
...
3. Decide whether RF-DETR/SAM3 actual inference is in scope for this replacement; if yes, add Swift-backed predictor routes or explicit compatibility exclusions.
