...
- `SimpleTokenizer` executes dependency-free WordLevel `tokenizer.json`, WordPiece `tokenizer.json`/`vocab.txt`, Unigram `tokenizer.json` and binary SentencePiece `tokenizer.model` greedy fallbacks with SentencePiece-style `▁` metaspace decoding, `tokenizer.tiktoken` exact-piece catalogs, and limited catalog-backed BPE tokenization for whitespace-separated text with exact special-token matching and unknown-token fallback; non-Unigram SentencePiece model variants remain classified for backend implementation.
...
- `StructuredOutputValidator` parses generated text for JSON/JSON Schema requests and attaches diagnostic validation results to render reports, covering object roots, required properties, property types, enum values, array items, and `additionalProperties:false` without changing the client-facing API response body.
...
- `ResponseFormatPlan` classifies Ollama/OpenAI structured-output requests, including JSON mode, JSON Schema guidance, missing schema warnings, stream framing requirements, and a `JSONSchemaConstraintPlan` that extracts root types, required properties, property paths, enum paths, array paths, nesting depth, unsupported schema keywords, and whether the current backend can only do root/prefix guidance versus requiring a full grammar/DFA.
- `MLXVLMUpstreamBackend` adds real-backend structured-output guidance for JSON mode and JSON Schema roots, including root-token masking, first required-key prefix guidance when a simple object schema is present, and deterministic full-literal token/EOS forcing for strict schema subsets where every required value is fixed by `const`, `default`, or a single-value `enum`. Full general JSON Schema grammar/DFA constrained decoding remains a tracked gap, but the schema constraint plan now makes the uncovered enum/array/nesting surface explicit in preflight output.
...
- `scripts/verify_real_qwen25vl_video_smoke.sh` is the video-capable real-backend gate. It builds with upstream Hugging Face downloader support, defaults to `mlx-community/Qwen2.5-VL-3B-Instruct-3bit`, creates tiny local PNG/MP4 fixtures with `ffmpeg`, starts the Swift server, and verifies upstream tokenization, OpenAI image data URI input, OpenAI video file URL input, and OpenAI SSE streaming all reach real MLX generation without Python. By default it requires `MLXVLM_REAL_QWEN25VL_MODEL` to point at a local model directory; set `MLXVLM_REAL_QWEN25VL_ALLOW_REMOTE=1` to let the smoke fetch the Hugging Face file list and download missing files with resume-capable `curl -C -` before starting the Swift server. The gate passed locally with `MLXVLM_REAL_QWEN25VL_MODEL=/Users/naen/Models/mlx-community-Qwen2.5-VL-3B-Instruct-3bit`, and the `ALLOW_REMOTE=1` path passed using the same completed download directory.
