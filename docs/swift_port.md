# MLX-VLM Swift Port Plan

This repository is still primarily the upstream Python MLX-VLM codebase. The Swift port should preserve Python model compatibility first, then replace implementation layers one by one.
...
- Server request shapes for `/chat/completions`, `/v1/chat/completions`, `/responses`, `/v1/responses`, and `/generate`.
- Ollama-style discovery endpoints, starting with `/api/tags` and `/api/show`.

Remote Hugging Face downloads are intentionally not implemented in the first Swift target. Hugging Face-style model IDs are resolved only when the model already exists in a local HF cache (`HUGGINGFACE_HUB_CACHE`, `HF_HOME`, `TRANSFORMERS_CACHE`, or the default `~/.cache/huggingface/hub` layout).

## Phase 0: Swift Compatibility Shell
...
- `AdapterMetadata` detects Python mlx-vlm LoRA adapter directories/files (`adapter_config.json` plus `adapters.safetensors`) separately from base model weights, preserving rank/alpha/dropout and safetensors readability for future adapter loading.
...
- `ChatTemplatePlan` reports whether the discovered chat template can use the current native Swift prompt renderer (`qwen-chat`/plain fallback) or needs a future Jinja/custom-template renderer for exact formatting.
...
- `GenerationTokenTextDecoder` turns sampled token IDs into incremental text deltas by reusing the simple detokenizer and tracking the decoded suffix, which gives the future decode loop a stable token-ID-to-chunk-text bridge.
- `TokenizerCatalogBuilder` also builds catalogs from sidecar `vocab.json` + `merges.txt` tokenizers, using `tokenizer_config.json` to recover unknown and added special tokens when `tokenizer.json` is absent.
...
- `SimpleTokenizer` executes dependency-free WordLevel `tokenizer.json` tokenization and limited catalog-backed BPE tokenization for whitespace-separated text with exact special-token matching and unknown-token fallback; production ByteLevel BPE, Unigram, WordPiece, and SentencePiece remain classified for backend implementation.
...
- `WeightTensorPayload` provides an exact byte-range reader for one safetensors tensor payload so the eventual MLX backend has a shared, validated source for array allocation.
...
- `GenerationRequestMetadata` also preserves raw Ollama `options` so backend-relevant settings that the compatibility shell does not execute yet, such as GPU/thread/mmap hints, remain available to the eventual Swift backend.
- OpenAI chat and Responses normalization preserves mlx-vlm server extensions such as `top_k`, `min_p`, `repetition_penalty`, `logit_bias`, thinking controls, logprob controls, resize shape, adapter path, and user identifiers for backend handoff.
- Chat message normalization preserves Python server message fields such as assistant reasoning, tool calls, tool-call IDs, function names, `output_text` content parts, and nested `input_audio` payloads.
...
- `EmbeddingRequest` normalizes Ollama `/api/embed`, legacy Ollama `/api/embeddings`, and OpenAI `/v1/embeddings` payloads into a shared embedding contract. Real MLX builds can load upstream `MLXEmbedders` for embedding-model directories and render Ollama/OpenAI embedding envelopes; VLM-only models still return typed unavailable reports.
- Ollama model-management operations (`/api/create`, `/api/pull`, `/api/push`, `/api/copy`, `/api/delete`) return typed unavailable reports in the compatibility shell instead of mutating local files or falling through to generic 404s.
...
- Ollama blob operations are recognized at `/api/blobs/{digest}`: `HEAD` returns a proper not-present status in the compatibility shell, while `POST` returns a typed unavailable report until blob storage/import is implemented.
...
- `MediaReferenceResolver` summarizes Ollama raw base64 images, OpenAI data URIs, local media paths, file URLs, and remote URLs without downloading remote data or echoing payloads back into prompts.
- `APIResponses` defines Ollama generate/chat, OpenAI chat completion, OpenAI Responses, OpenAI model list/retrieve, streaming chunk response shapes, and JSONL/SSE stream framing helpers for the eventual Swift backend output.
...
- `VLMGenerationRunner` wraps any future `VLMBackend` token stream with the shared assembler, so backend implementations only need to emit `GenerationChunk` values while Core handles stop filtering, prompt/completion usage, final `CompletedGeneration` construction, and rendered Ollama/OpenAI response handoff.
...
- `Package.swift` keeps the default compatibility build dependency-free, but supports `MLXVLM_ENABLE_MLX_BACKEND=1` to attach `MLX`, `MLXLMCommon`, `MLXLLM`, and `MLXVLM` products, plus `MLXVLM_ENABLE_TOKENIZER_INTEGRATIONS=1` to attach `MLXLMTokenizers` once `swift-tokenizers-mlx` can be fetched or vendored.
...
- `MLXWeightBackedModelContainer` is the backend-side container boundary after metadata and weight payload loading. It currently exposes a summary of prepared tensors, loaded arrays, and remaining module/generation blockers; a future implementation can extend this with live MLX module state.
...
- `MLXMetalLibrarySupport` prepares the MLX Metal runtime for SwiftPM debug executables by checking `MLXVLM_MLX_METALLIB_PATH`, common local Python `mlx` install locations, executable-adjacent paths, and SwiftPM bundle paths, then installing `mlx.metallib` as `default.metallib` before the real backend loads. If one destination is already populated, server startup can reuse it as the source and fill missing executable or bundle destinations.
- `MLXVLMUpstreamBackend` is the first real generation backend. When built with `MLXVLM_ENABLE_MLX_BACKEND=1`, `MLXVLM_ENABLE_TOKENIZER_INTEGRATIONS=1`, and `MLXVLM_ENABLE_REAL_MLX_API=1`, it loads a local model directory with `VLMModelFactory.shared.loadContainer(from:using:)`, converts normalized Swift chat requests into `MLXLMCommon.UserInput`, bridges OpenAI/Ollama-compatible tool schemas into upstream `ToolSpec` dictionaries, materializes OpenAI data URI and Ollama raw base64 media into temporary files for upstream `UserInput.Image.url`/`Video.url`, maps runtime options into `GenerateParameters`, streams upstream `Generation.chunk` values into the shared `GenerationChunk` contract, maps upstream `Generation.toolCall` values into shared tool-call chunks, maps upstream `GenerateCompletionInfo` into exact prompt/completion token usage, and renders the existing Ollama/OpenAI response envelopes.
...
- `GenerationPreflightPlanner` assembles the eventual backend handoff: prompt style, rendered prompt, media summaries, processor-aware image grid plans, RGB pixel preflight, generation parameters, compatibility checks, and blocking reasons.
...
- `GenerationSamplingPlan` classifies Ollama/OpenAI sampling controls into greedy, temperature, or Mirostat sampler intent, records enabled probability filters and penalties, and flags advanced sampler requirements such as `typical_p` and `tfs_z`.
- `GenerationLogitsSampler` provides a dependency-free Swift sampler for backend logits vectors, covering greedy argmax, temperature sampling, top-k, top-p, min-p, seeded randomness, repetition/presence/frequency penalties, and newline penalty handoff when a newline token ID is known.
- `GenerationLogitsDecodeExecutor` composes logits sampling, incremental token text decoding, and the decode loop into one dependency-free execution path that a future MLX forward implementation can feed with real logits rows.
...
- `GenerationEndpointRenderer` collects backend generation chunks into completed text/token usage and routes them through the Ollama/OpenAI response renderer, so the future MLX generator can hand off token deltas without duplicating API response code.
...
- `mlx-vlm-swift plan-model-load --model /path/to/model` prints the complete model-load handoff plan for an eventual MLX Swift backend.
...
- `mlx-vlm-swift sample-logits --logits 0.1,0.9,0.2 [--temperature n] [--top-k n] [--top-p n] [--min-p n] [--seed n] [--recent-token-ids ids]` runs the pure Swift logits sampler used by the future decode loop.
...
- `mlx-vlm-swift serve --model /path/to/model --port 11434` starts a server with `/health`, `/api/tags`, `/api/show`, `/models`, `/v1/models`, `/models/{id}`, and `/v1/models/{id}`; optional `--max-tokens`, `--temperature`, `--top-p`, `--top-k`, `--seed`, `--context-length`, and `--keep-alive` flags set request defaults. In a dependency-free build it returns typed generation diagnostics; in a real MLX build it loads `mlx-swift-vlm` and serves generated text through the same routes.
- For real MLX builds, `stream:true` generation requests on `/api/generate`, `/api/chat`, and `/v1/chat/completions` use socket-level streaming rather than a pre-buffered response body: the server writes NDJSON/SSE headers without `Content-Length`, then flushes each backend chunk frame as it arrives.
...
- The server also exposes `/backend/status`, Ollama-style `/api/version`, `/api/ps`, `/api/unload`, Ollama/OpenAI embedding routes, Ollama model-management routes, and CORS `OPTIONS` responses for UI/client integration. If real generation is unavailable, generation `501` responses keep the existing top-level compatibility fields and add a compact `mlxPipeline` readiness summary for settings UI diagnostics. In real MLX builds, `/health` also reports whether a separate embedding backend loaded for the selected model.
...
After real dependencies are present, enable the guarded array and upstream VLM bridge with `MLXVLM_ENABLE_REAL_MLX_API=1` as well. That compiles `MLXArrayWeightLoader`, `MLXGenerateParametersBridge`, `MLXVLMModelFactoryBridge`, and `MLXVLMUpstreamBackend`, which are intentionally separated from basic dependency-free checks because those checks should not pretend the real MLX, MLXLMCommon, MLXVLM, or MLXLMTokenizers APIs exist.
...
After at least one model generates correctly:
...
The Python package remains the source of truth until each gate passes for a specific architecture.
