...
MODEL="${MLXVLM_REAL_IMAGE_MODEL:-mlx-community/FastVLM-0.5B-bf16}"
ALLOW_REMOTE="${MLXVLM_REAL_IMAGE_ALLOW_REMOTE:-0}"
DOWNLOAD_DIR="${MLXVLM_REAL_IMAGE_DOWNLOAD_DIR:-}"
REQUEST_MODEL="${MLXVLM_REAL_IMAGE_REQUEST_MODEL:-vlm-image-smoke}"
SMOKE_LABEL="${MLXVLM_REAL_IMAGE_LABEL:-VLM image}"
...
if ! command -v ffmpeg >/dev/null 2>&1; then
  echo "real $SMOKE_LABEL smoke skipped: ffmpeg is required to create a tiny image fixture"
  exit 0
fi

if [[ "$MODEL" == */* && ! -d "$MODEL" && "$ALLOW_REMOTE" != "1" ]]; then
  echo "real $SMOKE_LABEL smoke skipped: set MLXVLM_REAL_IMAGE_MODEL to a local model directory, or set MLXVLM_REAL_IMAGE_ALLOW_REMOTE=1 to download $MODEL"
  exit 0
fi
...
TMP_ROOT="${MLXVLM_REAL_IMAGE_TMP_ROOT:-${TMPDIR:-/tmp}/mlx-vlm-swift-real-image-smoke-$$}"
PORT="${MLXVLM_REAL_IMAGE_PORT:-$((17000 + ($$ % 20000)))}"
SCRATCH_DIR="${MLXVLM_REAL_IMAGE_SCRATCH:-${TMPDIR:-/tmp}/mlx-vlm-swift-real-image-build}"
READY_TIMEOUT_SECONDS="${MLXVLM_REAL_IMAGE_READY_TIMEOUT:-1800}"
...
cleanup() {
  ...
  if [[ "$status" -eq 0 && "${MLXVLM_REAL_IMAGE_KEEP_TMP:-0}" != "1" ]]; then
    rm -rf "$TMP_ROOT"
