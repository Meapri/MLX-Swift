...
SCHEMA_PREFLIGHT_JSON="$(mktemp)"
"$BIN" preflight-generate --model "$MODEL_DIR" --api openai-responses --json '{"model":"verify","input":"schema","text":{"format":{"type":"json_schema","schema":{"type":"object","required":["answer","confidence"],"properties":{"answer":{"type":"string"},"confidence":{"type":"string","enum":["low","high"]},"items":{"type":"array","items":{"type":"object","required":["label"],"properties":{"label":{"type":"string"}}}}},"additionalProperties":false}}},"stream":true}' > "$SCHEMA_PREFLIGHT_JSON"
grep -q '"requiresSchemaGuidance" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"streamFraming" : "api-native-stream"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"schemaConstraints"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"deterministicLiteral" : "{\\"answer\\":\\"\\",\\"confidence\\":\\"low\\"}"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"grammarPlan"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"rootSymbol" : "object"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"deterministicPropertyOrder" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"structuralStateCount" : 6' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"enumLiteralPaths" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"additionalPropertiesFalsePaths" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"openStringPaths" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"$.answer"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"$.items\[\].label"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"openNumberPaths" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"finiteTokenDFAAlternativeCount" : 2' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"canCompileFiniteTokenDFA" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"finiteTokenDFAUsesScalarDefaults" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"canCompileOpenScalarGrammar" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"requiresTokenizerPrefixFiltering" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"requiresBackendTokenDFA" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"requiredProperties" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"answer"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"enumPaths" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"$.confidence"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"arrayPaths" : \[' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"$.items"' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"maxDepth" : 3' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"canConstrainRequiredPropertyPrefix" : true' "$SCHEMA_PREFLIGHT_JSON"
grep -q '"requiresFullGrammarDFA" : true' "$SCHEMA_PREFLIGHT_JSON"
